...
function App() {
  ...
  // Consume core auth states
  const currentUserEmail = useStore(appStore, state => state.currentUserEmail);
  const isAuthReady = useStore(appStore, state => state.isAuthReady);
  
  // Consume derived states
  const { filteredTodos, activeCount, filter, sortOrder, searchQuery } = useFilteredTodos();

  // Actions mapped safely with callbacks
  ...
  if (!isAuthReady) {
    return (
      <div className="min-h-screen bg-todo-light-bg dark:bg-todo-dark-bg flex items-center justify-center transition-colors">
         <p className="text-xl font-bold text-todo-light-text dark:text-todo-dark-text opacity-70 animate-pulse">Loading Session...</p>
      </div>
    );
  }

  const bgStyles = isDarkMode
    ? { backgroundImage: `url(${bgDarkDesktop})`, backgroundSize: '100% auto', backgroundPosition: 'center', opacity: 0.75 }
    : { backgroundImage: `url(${bgLightDesktop})`, backgroundSize: '100% auto', backgroundPosition: 'center', opacity: 0.75 };
  ...
  return (
    <div className="min-h-screen pt-16 px-6 pb-20 relative bg-todo-light-bg dark:bg-todo-dark-bg transition-colors duration-300">
      ...
      <div className="absolute top-0 left-0 w-full h-[300px] overflow-hidden">
        <div className="w-full h-full bg-no-repeat transition-all duration-300" style={bgStyles} />
        {/* Gradient Overlay */}
          ...
          <div className="flex gap-4 items-center justify-center">
            {['All', 'Active', 'Completed'].map(btn => (
              <button
                ...
                className={`font-bold transition-colors ${filter === btn ? 'text-todo-primary' : 'hover:text-todo-light-text dark:hover:text-todo-dark-text'}`}
              >
                {btn}
              </button>
            ))}
